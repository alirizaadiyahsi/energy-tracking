...
class MasterTestRunner:
    ...
    def check_prerequisites(self) -> Tuple[bool, List[str]]:
        ...
        if missing_packages:
            self.print_colored(
                f"❌ Missing packages: {', '.join(missing_packages)}", "91"
            )
            self.print_colored(
                "Install with: pip install " + " ".join(missing_packages), "93"
            )
            return False, missing_packages

        # Check if test files exist
        required_files = [self.tests_dir / "conftest.py", self.tests_dir / "pytest.ini"]
    ...
    def check_dependencies(
        self, suite_name: str, completed_suites: set
    ) -> Tuple[bool, List[str]]:
    ...
    async def run_all_tests(
        ...
        self.print_colored(
            f"Parallel execution: {'enabled' if parallel else 'disabled'}", "96"
        )
        ...
        for group_index, suite_group in enumerate(suite_groups):
            self.print_colored(
                f"\n📋 Executing group {group_index + 1}: {', '.join(suite_group)}",
                "95",
            )
            ...
            for suite_name in suite_group:
                ...
                if deps_ok:
                    ...
                    self.print_colored(
                        f"⏸️  Skipping {suite_name}: missing dependencies {missing_deps}",
                        "93",
                    )
                    result = TestResult(
            ...
            if len(ready_suites) > 1 and parallel:
                ...
                for suite_name, task in tasks:
                    ...
                    if result.passed:
                        self.print_colored(
                            f"✅ {suite_name} - PASSED ({result.duration:.2f}s)", "92"
                        )
                        completed_suites.add(suite_name)
                    else:
                        self.print_colored(
                            f"❌ {suite_name} - FAILED ({result.duration:.2f}s)", "91"
                        )
                        ...
                        if fail_fast:
                            self.print_colored(
                                "💥 Fail-fast enabled. Stopping execution.", "91"
                            )
                            return False
            else:
                # Run sequentially
        ...
        while remaining:
            ...
            for suite_name in list(remaining):
                ...
                if not suite.dependencies or all(
                    dep in processed for dep in suite.dependencies
                ):
            ...
            if not current_group:
                ...
                self.print_colored(
                    f"⚠️  Circular or missing dependencies for: {remaining}", "93"
                )
    ...
    def print_summary(self, total_duration: float):
        ...
        self.print_colored(
            f"Test Suites: {total_suites} total, {passed_suites} passed, {failed_suites} failed, {skipped_suites} skipped",
            "96",
        )

        success_rate = (passed_suites / total_suites * 100) if total_suites > 0 else 0
        self.print_colored(
            f"Success Rate: {success_rate:.1f}%", "92" if success_rate > 80 else "93"
        )
        ...
        for result in self.results:
            ...
            status_text = (
                "PASSED" if result.passed else "SKIPPED" if result.skipped else "FAILED"
            )
        ...
        if failed_suites > 0:
            ...
            self.print_colored(
                "  - Run individual test suites for detailed debugging", "93"
            )
...
async def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run only quick tests (unit + integration)"
    )
    parser.add_argument(
        "--full",
        action="store_true",
        help="Run all tests including performance and E2E",
    )
    ...
    if args.list:
        ...
        for name, suite in runner.test_suites.items():
            ...
            runner.print_colored(
                f"  {name:15} | {required_text:8} | {suite.description}", "96"
            )
        return
