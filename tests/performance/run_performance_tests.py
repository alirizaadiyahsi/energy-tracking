...
class PerformanceTestRunner:
    ...
    def check_prerequisites(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-c", "import locust"],
                capture_output=True,
                text=True
            )
            if result.returncode != 0:
                self.print_colored("❌ Locust not installed. Run: pip install locust", "91")
                return False
    ...
    def run_performance_test(
        ...
        command = [
            ...
            "--host", host,
            "--users", str(scenario_config.get("users", 10)),
            "--spawn-rate", str(scenario_config.get("spawn_rate", 2)),
            "--run-time", scenario_config.get("run_time", "60s"),
            "--html", str(self.results_dir / f"{results_prefix}.html"),
            "--csv", str(self.results_dir / results_prefix),
            "--loglevel", "INFO"
        ]
    ...
    def _analyze_results(self, results_prefix: str) -> None:
        ...
        if csv_file.exists():
            try:
                ...
                self.print_colored("⚠️  Install pandas for detailed analysis: pip install pandas", "93")
            except Exception as e:
    ...
    def _check_thresholds(self, df) -> None:
        ...
        if max_response > thresholds["max_response_time"]:
            self.print_colored(f"❌ Max response time exceeded: {max_response:.2f}ms > {thresholds['max_response_time']}ms", "91")
        else:
            self.print_colored(f"✅ Max response time within threshold: {max_response:.2f}ms", "92")
        ...
        if error_rate > thresholds["error_rate"]:
            self.print_colored(f"❌ Error rate exceeded: {error_rate:.2f}% > {thresholds['error_rate']}%", "91")
        else:
            self.print_colored(f"✅ Error rate within threshold: {error_rate:.2f}%", "92")
        
        # Check requests per second
        avg_rps = df['Requests/s'].mean()
        if avg_rps < thresholds["min_requests_per_second"]:
            self.print_colored(f"❌ Requests per second below threshold: {avg_rps:.2f} < {thresholds['min_requests_per_second']}", "91")
        else:
            self.print_colored(f"✅ Requests per second above threshold: {avg_rps:.2f}", "92")
    ...
    def run_load_test_suite(self, host: str = "http://localhost:8000") -> None:
        ...
        for scenario, success in results.items():
            ...
            self.print_colored(f"{scenario.upper()}: {status}", "92" if success else "91")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Energy Tracking Performance Test Runner")
    
    parser.add_argument("--scenario", choices=["light", "medium", "heavy", "stress", "spike"],
