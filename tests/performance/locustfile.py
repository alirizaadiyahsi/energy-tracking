...
class AuthServiceUser(HttpUser):
    ...
    def refresh_token(self):
        ...
        if self.auth_token:
            ...
            with self.client.post(
                "/auth/refresh", json=refresh_data, catch_response=True
            ) as response:
                if response.status_code == 200:
                    try:
                        response_data = response.json()
...
class AnalyticsUser(HttpUser):
    ...
    def get_dashboard_data(self):
        ...
        with self.client.get(
            "/analytics/dashboard", headers=headers, catch_response=True
        ) as response:
            if response.status_code == 200:
                # Check response time
...
class APIGatewayUser(HttpUser):
    ...
    def rate_limit_test(self):
        ...
        for i in range(10):
            with self.client.get("/api/test-endpoint", catch_response=True) as response:
...
class StressTestUser(HttpUser):
    ...
    def stress_endpoints(self):
        ...
        with self.client.post(
            "/data-ingestion/ingest", json=data, catch_response=True
        ) as response:
