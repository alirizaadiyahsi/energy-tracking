...
class E2ETestRunner:
    ...
    async def run_all_tests(self) -> bool:
        ...
        for i, (method_name, test_name) in enumerate(test_methods, 1):
            ...
            try:
                ...
                if success:
                    self.print_colored(f"✅ {test_name} - PASSED ({duration:.2f}s)", "92")
                    self.results.append(TestResult(test_name, True, duration))
                    passed_tests += 1
                else:
                    self.print_colored(f"❌ {test_name} - FAILED ({duration:.2f}s)", "91")
                    self.results.append(TestResult(test_name, False, duration))
                    
            except Exception as e:
                duration = time.time() - start_time
                self.print_colored(f"❌ {test_name} - ERROR: {str(e)} ({duration:.2f}s)", "91")
    ...
    def print_summary(self, passed: int, total: int) -> None:
        ...
        self.print_colored(f"Tests Passed: {passed}/{total} ({success_rate:.1f}%)", 
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code != 200:
                    self.print_colored(f"API Gateway health check failed: {response.status_code}", "91")
                    return False
                ...
                for service in services:
                    try:
                        response = await client.get(f"{self.api_gateway_url}/{service}/health")
                        if response.status_code == 200:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code not in [201, 409]:  # 409 if user already exists
                    self.print_colored(f"Registration failed: {response.status_code} - {response.text}", "91")
                    return False
    ...
    async def user_authentication_flow(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code != 200:
                    self.print_colored(f"Login failed: {response.status_code} - {response.text}", "91")
                    return False
                ...
                if response.status_code != 200:
                    self.print_colored(f"Token verification failed: {response.status_code}", "91")
                    return False
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code not in [201, 409]:
                    self.print_colored(f"Device creation failed: {response.status_code} - {response.text}", "91")
                    return False
                ...
                if response.status_code != 200:
                    self.print_colored(f"Device listing failed: {response.status_code}", "91")
                    return False
                ...
                # Get specific device
                response = await client.get(f"{self.devices_url}/{device_id}", headers=headers)
                
                if response.status_code != 200:
                    self.print_colored(f"Device retrieval failed: {response.status_code}", "91")
                    return False
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code not in [201, 202]:
                    self.print_colored(f"Data ingestion failed: {response.status_code} - {response.text}", "91")
                    return False
                ...
                if response.status_code not in [201, 202]:
                    self.print_colored(f"Batch ingestion failed: {response.status_code} - {response.text}", "91")
                    return False
    ...
    async def analytics_flow(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code not in [200, 404]:  # 404 acceptable if no data yet
                    self.print_colored(f"Device analytics failed: {response.status_code} - {response.text}", "91")
                    return False
                
                if response.status_code == 200:
                    analytics = response.json()
                    self.print_colored(f"  ✅ Device analytics retrieved: {analytics}", "92")
                else:
                    self.print_colored("  ✅ Device analytics endpoint accessible (no data yet)", "92")
                ...
                if response.status_code not in [200, 404]:
                    self.print_colored(f"Consumption trends failed: {response.status_code} - {response.text}", "91")
                    return False
                ...
                if response.status_code not in [200, 404]:
                    self.print_colored(f"Efficiency metrics failed: {response.status_code} - {response.text}", "91")
                    return False
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code not in [200, 204]:
                    self.print_colored(f"Logout failed: {response.status_code} - {response.text}", "91")
                    return False
                ...
                if response.status_code != 401:
                    self.print_colored(f"Token should be invalidated: {response.status_code}", "91")
                    return False
