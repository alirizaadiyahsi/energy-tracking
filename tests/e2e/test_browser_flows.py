...
class BrowserE2ETest:
    ...
    def __init__(self, base_url: str = "http://localhost:3000", headless: bool = True):
        ...
        # Test data
        self.test_user = {
            "email": self._get_config(
                "test_data", "test_user_email", "browser_test@example.com"
            ),
            "password": self._get_config(
                "test_data", "test_user_password", "BrowserTest123!"
            ),
            "first_name": self._get_config(
                "test_data", "test_user_first_name", "Browser"
            ),
            "last_name": self._get_config(
                "test_data", "test_user_last_name", "TestUser"
            ),
        }
                ...
                self.wait.until(
                    EC.any_of(
                        ...
                        EC.presence_of_element_located(
                            (
                                By.XPATH,
                                "//*[contains(text(), 'registered') or contains(text(), 'Registered')]",
                            )
                        ),
        ...
        try:
            ...
            email_field = self.wait.until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
                        "//input[@type='email' or @name='email' or @name='username' or @id='email' or @id='username']",
                    )
                )
            )
            email_field.clear()
        ...
        try:
            ...
            self.wait.until(
                EC.any_of(
                    ...
                    EC.presence_of_element_located(
                        (By.XPATH, "//*[contains(text(), 'Energy')]")
                    ),
                    EC.presence_of_element_located(
                        (By.XPATH, "//*[contains(text(), 'Devices')]")
                    ),
                )
            )
    ...
    async def test_device_creation(self) -> bool:
        ...
        try:
            ...
            try:
                devices_link = self.driver.find_element(
                    By.XPATH,
            ...
            name_field = self.wait.until(
                EC.presence_of_element_located(
                    (
                        By.XPATH,
                        "//input[@name='name' or @id='name' or @placeholder*='name']",
                    )
                )
            )
            name_field.clear()
            ...
            try:
                type_field = self.driver.find_element(
                    By.XPATH, "//select[@name='type'] | //input[@name='type']"
                )
            ...
            try:
                location_field = self.driver.find_element(
                    By.XPATH, "//input[@name='location' or @id='location']"
                )
            ...
            self.wait.until(
                EC.any_of(
                    EC.presence_of_element_located(
                        (
                            By.XPATH,
                            "//*[contains(text(), 'success') or contains(text(), 'created')]",
                        )
                    ),
                    EC.presence_of_element_located(
                        (By.XPATH, "//div[contains(text(), 'Browser Test Device')]")
                    ),
                )
            )
    ...
    async def test_device_management(self) -> bool:
        ...
        try:
            ...
            test_device = self.driver.find_element(
                By.XPATH, "//*[contains(text(), 'Browser Test Device')]"
            )
            ...
            self.wait.until(
                EC.any_of(
                    EC.presence_of_element_located(
                        (By.XPATH, "//*[contains(text(), 'Device Details')]")
                    ),
                    EC.presence_of_element_located(
                        (By.XPATH, "//*[contains(text(), 'Browser Test Device')]")
                    ),
    ...
    async def test_analytics_view(self) -> bool:
        ...
        try:
            ...
            try:
                analytics_link = self.driver.find_element(
                    By.XPATH,
            ...
            self.wait.until(
                EC.any_of(
                    ...
                    EC.presence_of_element_located(
                        (
                            By.XPATH,
                            "//*[contains(text(), 'Chart') or contains(text(), 'Graph')]",
                        )
                    ),
                    EC.presence_of_element_located(
        ...
        try:
            ...
            self.wait.until(
                EC.any_of(
                    ...
                    EC.presence_of_element_located(
                        (By.XPATH, "//input[@type='email'] | //input[@type='text']")
                    ),
                )
            )
        ...
        try:
            ...
            logout_element = self.wait.until(
                EC.element_to_be_clickable(
                    (
                        By.XPATH,
                        "//a[contains(text(), 'Logout') or contains(text(), 'Sign Out')] | //button[contains(text(), 'Logout') or contains(text(), 'Sign Out')]",
                    )
                )
            )
...
async def main():
    ...
    try:
        ...
        tester.print_colored(
            f"Tests Passed: {passed}/{total}", "92" if passed == total else "91"
        )
