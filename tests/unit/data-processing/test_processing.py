    ...
    def test_data_filtering(self):
        ...
        def filter_readings(readings, filters):
            ...
            if "device_id" in filters:
                filtered = [
                    r for r in filtered if r.get("device_id") == filters["device_id"]
                ]
            ...
            if "min_power" in filters:
                filtered = [
                    r for r in filtered if r.get("power", 0) >= filters["min_power"]
                ]

            if "max_power" in filters:
                filtered = [
                    r for r in filtered if r.get("power", 0) <= filters["max_power"]
                ]
        ...
        # Filter by time range
        morning_readings = filter_readings(
            readings,
    ...
    def test_duplicate_detection(self):
        ...
        def detect_duplicates(readings):
            ...
            for i, reading in enumerate(readings):
                ...
                if key in seen:
                    duplicates.append(
                        {"index": i, "reading": reading, "duplicate_of": key}
                    )
                else:
                    seen.add(key)
    ...
    def test_data_completeness(self):
        ...
        def check_completeness(readings, required_fields):
            ...
            for i, reading in enumerate(readings):
                ...
                for field in required_fields:
                    if (
                        field not in reading
                        or reading[field] is None
                        or reading[field] == ""
                    ):
    ...
    def test_data_consistency(self):
        ...
        def check_consistency(readings):
            ...
            for i, reading in enumerate(readings):
                ...
                if all(
                    field in reading
                    for field in ["power", "voltage", "current", "power_factor"]
                ):
                    expected_power = (
                        reading["voltage"]
                        * reading["current"]
                ...
                if i > 0:
                    ...
                    previous_time = datetime.fromisoformat(
                        readings[i - 1]["timestamp"].replace("Z", "+00:00")
                    )
