...
async def create_processing_job(
    ...
    try:
        ...
        if job_data.job_type == "aggregation":
            background_tasks.add_task(processing_service.process_energy_aggregation, db_job.id)
...
async def get_energy_metrics(
    ...
    try:
        ...
        query = query.order_by(EnergyMetrics.period_start.desc()).limit(limit).offset(offset)
        result = await db.execute(query)
...
async def trigger_aggregation(
    ...
    try:
        ...
        background_tasks.add_task(processing_service.process_energy_aggregation, job_data.parameters)
...
async def get_processing_stats(
    ...
    try:
        ...
        pending_jobs_query = select(ProcessingJob).where(ProcessingJob.status == "pending")
        pending_jobs_result = await db.execute(pending_jobs_query)
        pending_jobs = len(pending_jobs_result.scalars().all())
        
        completed_jobs_query = select(ProcessingJob).where(ProcessingJob.status == "completed")
        completed_jobs_result = await db.execute(completed_jobs_query)
        completed_jobs = len(completed_jobs_result.scalars().all())
        
        failed_jobs_query = select(ProcessingJob).where(ProcessingJob.status == "failed")
