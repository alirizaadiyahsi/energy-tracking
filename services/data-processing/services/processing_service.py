...
class ProcessingService:
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                raw_data = await self.influx_service.get_energy_data(
                    device_id=device_id, start_time=start_date, end_time=end_date
                )
                ...
                logger.info(
                    f"Energy aggregation completed. Created {len(aggregations)} aggregations"
                )
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                recent_metrics = await db.execute(
                    select(EnergyMetrics)
                    .where(
                        EnergyMetrics.period_start
                ...
                for device_id, device_data in df.groupby("device_id"):
                    ...
                    features = device_data[
                        ["total_energy", "avg_power", "max_power"]
                    ].values
                    ...
                    for idx, (metric_idx, row) in enumerate(device_data.iterrows()):
                        ...
                        for metric in metrics:
                            if (
                                ...
                                and metric.period_start == row["period_start"]
                            ):
                ...
                logger.info(
                    f"Anomaly detection completed. Found {anomalies_detected} anomalies"
                )
    ...
    async def cleanup_old_data(self, job_id):
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                logger.info(
                    f"Data cleanup completed. Deleted {deleted_jobs} jobs and {deleted_metrics} metrics"
                )
    ...
    def _aggregate_by_period(
        self, raw_data: List[Dict], period_type: str
    ) -> List[Dict]:
        ...
        for device_id, device_data in df.groupby("device_id"):
            ...
            resampled = (
                device_data.resample(freq)
                .agg({"power": ["mean", "max", "min"], "energy": "sum"})
                .dropna()
            )

            for period_start, row in resampled.iterrows():
                period_end = period_start + pd.Timedelta(
                    hours=1 if period_type == "hourly" else 24
                )
