...
class SecurityReportGenerator:
    ...
    def parse_safety_report(self, report_file: Path) -> None:
        ...
        try:
            ...
            for vulnerability in safety_data:
                ...
                self.security_data['categories']['dependency_vulnerabilities'].append(issue)
        ...
        try:
            ...
            for vuln_id, vulnerability in npm_data.get('vulnerabilities', {}).items():
                issue = {
                    'tool': 'npm audit',
                    'category': 'dependency',
                    'severity': self._map_npm_severity(vulnerability.get('severity', 'moderate')),
                    'package': vulnerability.get('name', vuln_id),
                    'range': vulnerability.get('range', ''),
                    'title': vulnerability.get('title', 'Node.js dependency vulnerability'),
            ...
            for run in sarif_data.get('runs', []):
                for result in run.get('results', []):
                    ...
                    issue = {
                        ...
                        'rule_id': rule_id,
                        'title': rule.get('shortDescription', {}).get('text', 'Container vulnerability'),
                    ...
                    self.security_data['categories']['container_vulnerabilities'].append(issue)
                    self._update_severity_count(issue['severity'])
            ...
            for run in sarif_data.get('runs', []):
                for result in run.get('results', []):
                    ...
                    issue = {
                        ...
                        'severity': self._map_sarif_severity(result.get('level', 'warning')),
                        'rule_id': rule_id,
                        'title': rule.get('shortDescription', {}).get('text', 'Code security issue'),
                        'description': rule.get('fullDescription', {}).get('text', ''),
                        'file': self._get_sarif_location_file(result),
                        'line': self._get_sarif_location_line(result),
                        'references': self._get_sarif_help_uri(rule)
    ...
    def parse_license_reports(self) -> None:
        ...
        if python_licenses_file.exists():
            try:
                ...
                problematic_licenses = ['GPL v3', 'AGPL v3', 'GPL v2', 'GPL-3.0', 'AGPL-3.0', 'GPL-2.0']
    ...
    def collect_reports(self) -> None:
        ...
        logger.info(f"Found {self.security_data['summary']['total']} total security issues")
...
<body>
    ...
    def _generate_category_sections(self) -> str:
        ...
        category_info = {
            'dependency_vulnerabilities': ('📦 Dependency Vulnerabilities', 'Vulnerabilities in third-party dependencies'),
            'container_vulnerabilities': ('🐳 Container Vulnerabilities', 'Security issues in container images'),
            'code_vulnerabilities': ('💻 Code Vulnerabilities', 'Security issues in application code'),
            'secret_exposures': ('🔑 Secret Exposures', 'Exposed secrets and credentials'),
            'infrastructure_issues': ('🏗️ Infrastructure Issues', 'Infrastructure and configuration security issues'),
        ...
        for category, issues in self.security_data['categories'].items():
            if category in category_info:
                ...
                sections_html += self._generate_category_section(title, description, issues)
        
        return sections_html
    
    def _generate_category_section(self, title: str, description: str, issues: List[Dict]) -> str:
