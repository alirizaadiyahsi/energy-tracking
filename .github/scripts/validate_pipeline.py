...
class PipelineValidator:
    ...
    def validate_workflows(self) -> None:
        ...
        for workflow_file, description in expected_workflows.items():
            ...
            if not workflow_path.exists():
                self.validation_results['issues'].append(f"Missing workflow file: {workflow_file}")
                continue
            ...
            try:
                ...
                validation = self._validate_workflow_structure(workflow_data, workflow_file)
                ...
                self.validation_results['issues'].append(f"Failed to parse {workflow_file}: {e}")
                self.validation_results['workflows'][workflow_file] = {
                    'description': description,
                    'status': 'error',
                    'error': str(e)
    ...
    def validate_scripts(self) -> None:
        ...
        for script_file, description in expected_scripts.items():
            ...
            if not script_path.exists():
                self.validation_results['issues'].append(f"Missing script file: {script_file}")
                continue
            ...
            try:
                ...
                if not has_main:
                    self.validation_results['issues'].append(f"{script_file}: Missing main() function")
                    
            except Exception as e:
                self.validation_results['issues'].append(f"Failed to validate {script_file}: {e}")
                self.validation_results['scripts'][script_file] = {
                    'description': description,
                    'status': 'error',
                    'error': str(e)
    ...
    def validate_config_files(self) -> None:
        ...
        for config_file, description in config_files.items():
            ...
            if not config_path.exists():
                self.validation_results['issues'].append(f"Missing config file: {config_file}")
                continue
            ...
            try:
                ...
                self.validation_results['issues'].append(f"Invalid config file {config_file}: {e}")
                self.validation_results['config_files'][config_file] = {
                    'description': description,
                    'status': 'invalid',
                    'error': str(e)
    ...
    def check_dependencies(self) -> None:
        ...
        if not docker_files:
            self.validation_results['issues'].append("No Dockerfile found in the repository")
    ...
    def validate_pipeline(self) -> Dict[str, Any]:
        ...
        logger.info(f"Pipeline validation completed: {self.validation_results['summary']['overall_status']}")
        
        return self.validation_results
    
    def _validate_workflow_structure(self, workflow_data: Dict, workflow_file: str) -> Dict:
        ...
        if 'on' not in workflow_data and True not in workflow_data:
            ...
            validation['triggers'] = list(trigger_data.keys()) if isinstance(trigger_data, dict) else [trigger_data]
        ...
        if 'jobs' not in workflow_data:
            ...
            for job_name, job_data in workflow_data['jobs'].items():
                if 'runs-on' not in job_data:
                    validation['issues'].append(f"Job '{job_name}' missing 'runs-on' field")
                    validation['valid'] = False
                
                if 'steps' not in job_data:
                    validation['issues'].append(f"Job '{job_name}' missing 'steps' field")
                    validation['valid'] = False
        
        # Workflow-specific validations
        if workflow_file == 'ci.yml':
            required_jobs = ['code-quality', 'unit-tests', 'integration-tests']
            missing_jobs = [job for job in required_jobs if job not in validation['jobs']]
            if missing_jobs:
            ...
            missing_jobs = [job for job in required_jobs if job not in validation['jobs']]
            if missing_jobs:
                validation['issues'].append(f"Missing required security jobs: {missing_jobs}")
    ...
    def _calculate_completion_percentage(self) -> float:
        ...
        actual_items = {
            'workflows': len([w for w in self.validation_results['workflows'].values() if w.get('status') == 'valid']),
            'scripts': len([s for s in self.validation_results['scripts'].values() if s.get('status') == 'valid']),
            'config_files': len([c for c in self.validation_results['config_files'].values() if c.get('status') in ['exists', 'valid']])
        }
    ...
    def print_summary(self) -> None:
        ...
        for workflow, info in results['workflows'].items():
            status_icon = "✅" if info['status'] == 'valid' else "❌" if info['status'] == 'invalid' else "⚠️"
