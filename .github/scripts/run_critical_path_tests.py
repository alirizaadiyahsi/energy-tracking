...
class CriticalPathTester:
    ...
    async def run_all_critical_tests(
        self, timeout: int = 300, retry_attempts: int = 3
    ) -> bool:
        ...
        if self.frontend_url:
            critical_tests.append(
                ("test_frontend_accessibility", "Frontend Accessibility")
            )
                ...
                try:
                    ...
                    if success:
                        ...
                        if attempt < retry_attempts - 1:
                            print(
                                f"    ⚠️  {test_name} failed, retrying... (attempt {attempt + 1}/{retry_attempts})"
                            )
                            await asyncio.sleep(2)
                        else:
                            print(
                                f"    ❌ {test_name} failed after {retry_attempts} attempts"
                            )
    ...
    async def test_system_health(self) -> bool:
        ...
        try:
            ...
            self.results.append(
                TestResult(
                    name="System Health Check", passed=False, duration=0, error=str(e)
                )
            )
            return False
    ...
    async def test_user_authentication(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                try:
                    await client.post(
                        f"{self.api_url}/auth/register", json=self.test_user
                    )
                except:
            ...
            self.results.append(
                TestResult(
                    name="User Authentication", passed=False, duration=0, error=str(e)
                )
            )
            return False
    ...
    async def test_device_operations(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                response = await client.post(
                    f"{self.api_url}/devices/", json=device_data, headers=headers
                )
            ...
            self.results.append(
                TestResult(
                    name="Device Operations", passed=False, duration=0, error=str(e)
                )
            )
            return False
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                success = response.status_code in [
                    ...
                    202,
                    404,
                ]  # 404 acceptable if endpoint not available
            ...
            self.results.append(
                TestResult(
                    name="Data Ingestion", passed=False, duration=0, error=str(e)
                )
            )
            return False
    ...
    async def test_analytics_endpoints(self) -> bool:
        ...
        try:
            ...
            self.results.append(
                TestResult(
                    name="Analytics Endpoints", passed=False, duration=0, error=str(e)
                )
            )
            return False
    ...
    async def test_api_performance(self) -> bool:
        ...
        try:
                ...
                self.results.append(
                    TestResult(
                        ...
                        details={
                            ...
                            "success_rate": len(successful_responses)
                            / len(responses)
                            * 100,
                        },
                    )
            ...
            self.results.append(
                TestResult(
                    name="API Performance", passed=False, duration=0, error=str(e)
                )
            )
            return False
